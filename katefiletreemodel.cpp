...
class ProxyItem {
  ...
  public:
    ...
    /*******************************************************/
    enum Type { Directory, File, Bookmark };
    /*******************************************************/
    ...
    int addBookmarkChild(ProxyItem *item); /******************************/
  ...
  private:
    ...
    /************************/
    QList<ProxyItem*> m_bookmark_children;
    int m_bookmark_children_count;
    bool m_bookmark;
    Type m_type;
    /************************/
    int m_row;
...
ProxyItem::ProxyItem(QString d, ProxyItemDir *p, ProxyItem::Flags f)
  : m_path(d), m_parent(p), m_row(-1), m_flags(f), m_doc(0), m_bookmark(false), m_type(Directory)
{
...
/********************************************************************/
int ProxyItem::addBookmarkChild(ProxyItem *item)
{  
  int item_row = m_bookmark_children.count();
  item->m_row = item_row;

  m_bookmark_children.append(item);
  item->m_parent = static_cast<ProxyItemDir*>(this);
...
{
  /* ako ima decu onda je folder -> treba promeniti */
  int sum = 0;
  QList<ProxyItem*>::iterator i;
  for (i = m_children.begin(); i != m_children.end(); i++) {
    sum += ((*i)->m_bookmark_children_count);
  }
   if(m_children.count()-sum > 0 || m_children.count() == 0){
    //std::cout << "\n\nKoliko dece:\t" << m_children.count();
    //std::cout << "\nKoliko bookmarkova:\t" << m_sum << "\n\n";
    return KIcon("folder");
  }
...
KateFileTreeModel::KateFileTreeModel(QObject *p,QMap<KTextEditor::Document*,QVariantList>* bookmap)
  : QAbstractItemModel(p),
    m_root(new ProxyItemDir(QString("m_root"), 0)),
    m_bookmap(bookmap)
{

  qDebug()<<"KateFileTreeModel::KateFileTreeModel";
  qDebug()<<*bookmap;
  qDebug()<<"}";
...
void KateFileTreeModel::setupIcon(ProxyItem *item)
{
  ...
  if(item->m_type == ProxyItem::Directory)
    icon_name = "folder";
